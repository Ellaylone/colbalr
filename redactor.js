        ...
        build: function () {
            return {
                ...
                plugins: function () {
                    ...
                    if (!RedactorPlugins) var RedactorPlugins = {'marker': (function()
{
    return {
        init: function ()
        {
            var button = this.button.add('link', 'marker', 'Marker');
            this.button.addCallback(button, this.marker.insertModal);
        },
        insertModal: function()
        {
            this.modal.addTemplate('input', this.marker.getTemplate());
            this.modal.load('input', 'Введите название маркера', 600);
            $('#redactor-insert-marker').focus();
            var ab = this.modal.createActionButton('Добавить', 'insertMarker');
            ab[0].addEventListener('click', this.marker.insertMarker);
            this.modal.show();
        },
        insertMarker: function(e)
        {
            var markerName = document.querySelector('#redactor-insert-marker').value;
            if(markerName != ''){
                this.modal.close();
                this.focus.setEnd();
                this.insert.html('<a name="' + markerName + '" class="hidden-anchor">' + markerName + '</a>');
            }
        },
        getTemplate: function()
        {
            return String()
                + '<div class="modal-section" id="redactor-modal-marker-insert">'
                + '<section>'
                + '<label>Название</label>'
                + '<input type="text" id="redactor-insert-marker" />'
                + '</section>'
                + '</div>';
        }
    };
